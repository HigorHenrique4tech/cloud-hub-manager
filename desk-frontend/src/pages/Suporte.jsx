/**
 * Suporte.jsx — Admin/Helpdesk ticket management panel (Desk portal version).
 * Route: /painel — accessible to is_admin or is_helpdesk users only.
 */
...
const PRIORITY_DOT   = { low: 'bg-gray-400', normal: 'bg-blue-500', high: 'bg-orange-500', urgent: 'bg-red-600' };
    ...
    <div className={`flex gap-2 ${isOwn ? 'flex-row-reverse' : 'flex-row'}`}>
      ...
      <div className={`max-w-[75%] flex flex-col gap-0.5 ${isOwn ? 'items-end' : 'items-start'}`}>
        <div className={`flex items-center gap-1.5 text-[11px] text-gray-400 ${isOwn ? 'flex-row-reverse' : ''}`}>
          ...
          {isInternal && (
            <span className="px-1.5 py-0.5 rounded text-[10px] bg-yellow-100 text-yellow-700 dark:bg-yellow-900/20 dark:text-yellow-400 border border-yellow-200 dark:border-yellow-800">interna</span>
          )}
        </div>
        <div className={`px-3 py-2 rounded-xl text-sm whitespace-pre-wrap break-words ${
          isOwn ? 'bg-primary text-white rounded-tr-sm'
          : isInternal ? 'bg-yellow-50 dark:bg-yellow-900/10 border border-yellow-200 dark:border-yellow-800 text-yellow-800 dark:text-yellow-300 rounded-tl-sm'
...
function TicketDetailPanel({ ticket, onClose, currentUser }) {
  ...
  const statusMut = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['admin-tickets'] }); qc.invalidateQueries({ queryKey: ['admin-ticket-detail', ticket.id] }); },
  });
  ...
  const handleSend = () => { if (reply.trim()) sendMut.mutate({ content: reply.trim(), is_internal: isInternal }); };
  ...
  return (
    <div className="flex flex-col h-full border-l border-gray-200 dark:border-gray-700">
      <div className="flex items-start justify-between px-4 py-3 border-b border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-0.5">
            ...
            <span className={`inline-flex items-center px-1.5 py-0.5 rounded-full text-[11px] font-medium border ${sCfg.badge}`}>{sCfg.short}</span>
          </div>
      ...
      <div className="px-4 py-2 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50 flex flex-wrap gap-3 text-xs text-gray-500 dark:text-gray-400">
        {t.creator && <span className="flex items-center gap-1"><User className="w-3 h-3" /><span className="font-medium text-gray-700 dark:text-gray-300">{t.creator.name}</span><span>({t.creator.email})</span></span>}
        {t.organization && <span className="flex items-center gap-1"><Building2 className="w-3 h-3" />{t.organization.name}</span>}
        {t.workspace && <span className="flex items-center gap-1"><Layers className="w-3 h-3" />{t.workspace.name}</span>}
        <span className="flex items-center gap-1"><Clock className="w-3 h-3" />{fmtFull(t.created_at)}</span>
      </div>
      ...
      <div className="px-4 py-2 border-b border-gray-200 dark:border-gray-700 flex gap-2 flex-wrap items-center">
        ...
        {['open', 'in_progress', 'waiting_client', 'resolved', 'closed'].map((s) => {
          ...
          return (
            <button key={s} onClick={() => statusMut.mutate(s)} disabled={t.status === s || statusMut.isPending}
              className={`px-2 py-0.5 rounded-full text-[11px] font-medium border transition-all ${t.status === s ? cfg.badge + ' ring-1 ring-offset-1 ring-current' : 'border-gray-200 dark:border-gray-600 text-gray-500 dark:text-gray-400 hover:border-gray-400'}`}>
              {cfg.short}
            </button>
          );
        })}
        <span className="text-xs font-medium text-gray-500 dark:text-gray-400 ml-2 mr-1">Prioridade:</span>
        <select value={t.priority} onChange={(e) => priorityMut.mutate(e.target.value)}
          ...
          {['low', 'normal', 'high', 'urgent'].map((p) => <option key={p} value={p}>{PRIORITY_LABEL[p]}</option>)}
        </select>
      </div>

      <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-3 bg-gray-50/50 dark:bg-gray-900/20">
        {messages.length === 0 && <div className="text-center text-xs text-gray-400 py-8">Nenhuma mensagem ainda.</div>}
      ...
      <div className="p-3 border-t border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 space-y-2">
        <textarea value={reply} onChange={(e) => setReply(e.target.value)}
          ...
          className={`w-full rounded-lg border px-3 py-2 text-sm focus:outline-none resize-none transition-colors ${isInternal ? 'border-yellow-300 dark:border-yellow-700 bg-yellow-50 dark:bg-yellow-900/10 text-yellow-900 dark:text-yellow-100 placeholder-yellow-500 focus:border-yellow-500' : 'border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:border-primary'}`} />
        {replyError && <p className="text-xs text-red-600 dark:text-red-400 flex items-center gap-1"><AlertTriangle className="w-3.5 h-3.5" /> {replyError}</p>}
        <div className="flex items-center justify-between">
          <label className="flex items-center gap-1.5 cursor-pointer select-none">
            <input type="checkbox" checked={isInternal} onChange={(e) => setIsInternal(e.target.checked)} className="rounded border-gray-300 text-yellow-500 focus:ring-yellow-400" />
          ...
          <button onClick={handleSend} disabled={sendMut.isPending || !reply.trim()}
...
function TicketRow({ ticket, isSelected, onClick }) {
  ...
  return (
    <button onClick={onClick}
      className={`w-full text-left px-4 py-3 border-b border-gray-100 dark:border-gray-700/50 transition-colors ${isSelected ? 'bg-primary/5 border-l-2 border-l-primary dark:bg-primary/10' : 'hover:bg-gray-50 dark:hover:bg-gray-700/30 border-l-2 border-l-transparent'}`}>
      <div className="flex items-start gap-2">
        <StatusIcon className={`w-4 h-4 mt-0.5 flex-shrink-0 ${sCfg.cls}`} />
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-1.5 mb-0.5">
            <span className="font-mono text-[11px] font-bold text-primary">{fmtTicketId(ticket)}</span>
          ...
          <div className="flex items-center gap-2 mt-0.5 text-xs text-gray-500 dark:text-gray-400 flex-wrap">
            {ticket.creator && <span className="flex items-center gap-0.5"><User className="w-3 h-3" />{ticket.creator.name}</span>}
            {ticket.organization && <span className="flex items-center gap-0.5"><Building2 className="w-3 h-3" />{ticket.organization.name}</span>}
...
const Suporte = () => {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => supportService.adminList({ status: activeQueue || undefined, search: search || undefined, page_size: 50 }),
  ...
  return (
    <div className="flex h-screen bg-gray-50 dark:bg-gray-900">
      <DeskSidebar />
      {/* Full-height 3-panel Jira layout */}
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="w-52 flex-shrink-0 border-r border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 flex flex-col">
          ...
          <nav className="flex-1 overflow-y-auto py-2">
            ...
            {QUEUES.map((q) => {
              ...
              return (
                <button key={q.id} onClick={() => { setActiveQueue(q.id); setSelectedTicket(null); }}
                  className={`w-full flex items-center gap-2 px-4 py-2 text-sm transition-colors ${isActive ? 'bg-primary/10 text-primary font-semibold' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700/50'}`}>
        ...
        <div className={`flex flex-col border-r border-gray-200 dark:border-gray-700 bg-slate-50 dark:bg-slate-800/60 ${selectedTicket ? 'w-80 flex-shrink-0' : 'flex-1'}`}>
          ...
          <div className="flex-1 overflow-y-auto">
            {isLoading ? (
              ...
              tickets.map((t) => (
                <TicketRow key={t.id} ticket={t} isSelected={selectedTicket?.id === t.id} onClick={() => setSelectedTicket(t)} />
              ))
            )}
          </div>
        </div>
        ...
        {selectedTicket ? (
          <div className="flex-1 flex flex-col bg-white dark:bg-gray-800 min-w-0">
            <TicketDetailPanel ticket={selectedTicket} onClose={() => setSelectedTicket(null)} currentUser={user} />
          </div>
        ) : (
