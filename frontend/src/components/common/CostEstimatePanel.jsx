...
// ── Pricing tables (us-east-1 / East US, On-Demand, Linux) ───────────────────
...
const AZURE_VM_HOURLY = {
  ...
  'Standard_NC6s_v3':3.06,'Standard_NC12s_v3':6.12,
};
const AZURE_DISK_PER_GB = {
...
function computeEstimate(type, form) {
  if (type === 'ec2') {
    const rate = EC2_HOURLY[form.instance_type];
    if (!rate) return null;
    const compute = rate * HOURS;
    const items = [{ label: `Compute (${form.instance_type})`, amount: compute }];
    (form.volumes || []).forEach(v => {
      const gb = v.volume_size_gb || 8;
      const cost = gb * (EBS_PER_GB[v.volume_type] ?? 0.10);
      items.push({ label: `EBS ${v.volume_type || 'gp2'} (${gb} GB)`, amount: cost });
    });
    const monthly = items.reduce((s, i) => s + i.amount, 0);
    return { monthly, hourly: monthly / HOURS, items };
  }
  ...
  if (type === 'rds') {
    ...
    const gb = form.allocated_storage || 20;
    const storage = gb * (RDS_STORAGE[form.storage_type] ?? 0.115);
    const items = [
      { label: `Instância (${form.db_instance_class}${form.multi_az ? ', Multi-AZ' : ''})`, amount: compute },
      { label: `Storage ${form.storage_type || 'gp2'} (${gb} GB)`, amount: storage },
    ];
    const monthly = compute + storage;
    return { monthly, hourly: monthly / HOURS, items };
  }
  ...
  if (type === 'azure-vm') {
    ...
    const diskCost = diskGb * (AZURE_DISK_PER_GB[form.os_disk_type] ?? 0.04);
    const items = [
      { label: `VM (${form.vm_size})`, amount: compute },
      { label: `OS Disk ${form.os_disk_type || 'Standard_LRS'} (${diskGb} GB)`, amount: diskCost },
    ];
    (form.data_disks || []).forEach((d, idx) => {
      const cost = (d.size_gb || 128) * (AZURE_DISK_PER_GB[d.type] ?? 0.04);
      items.push({ label: `Data Disk ${idx + 1} (${d.size_gb || 128} GB)`, amount: cost });
    });
    const monthly = items.reduce((s, i) => s + i.amount, 0);
    return { monthly, hourly: monthly / HOURS, items };
  }
  ...
  if (type === 'azure-sql') {
    ...
    return {
      monthly,
      hourly: monthly / HOURS,
      items: [{ label: `Azure SQL Database (${form.sku_name})`, amount: monthly }],
    };
  }
  ...
  if (type === 'azure-app-service') {
    ...
    return {
      monthly,
      hourly: monthly / HOURS,
      items: [{ label: `App Service Plan (${form.plan_sku})`, amount: monthly }],
    };
  }
...
const CostEstimatePanel = ({ type, form }) => {
  const estimate = computeEstimate(type, form);
  ...
  return (
    <div className="border-t border-dashed border-gray-200 dark:border-gray-700 pt-5 mt-2">
      <div className="rounded-xl overflow-hidden border border-emerald-200 dark:border-emerald-800 shadow-sm">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-xs opacity-60">Baseado em preços de referência</p>
              </div>
            </div>
            <div className="text-right">
              {isFree ? (
                ...
                <>
                  <div className="text-2xl font-bold">~${monthly.toFixed(2)}<span className="text-sm font-normal opacity-75">/mês</span></div>
        ...
        <div className="bg-white dark:bg-gray-800/50 px-5 py-3 space-y-2">
          {items.map((item, i) => (
            <div key={i} className="flex items-center justify-between">
              ...
              <span className={`text-sm font-semibold ${item.amount === 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-gray-900 dark:text-gray-100'}`}>
        ...
        {/* Disclaimer */}
        <div className="px-5 py-2.5 bg-emerald-50 dark:bg-emerald-950/30 border-t border-emerald-100 dark:border-emerald-900">
          <p className="text-xs text-emerald-700 dark:text-emerald-400 flex items-start gap-1.5">
            <Info className="w-3.5 h-3.5 flex-shrink-0 mt-0.5" />
            Valores estimados (us-east-1 / East US, Linux, On-Demand). Preços reais variam por região, sistema operacional e descontos aplicáveis.
          </p>
        </div>
      </div>
    </div>
