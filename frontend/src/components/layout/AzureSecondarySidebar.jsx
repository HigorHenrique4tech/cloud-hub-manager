...
import { LayoutGrid, MonitorPlay, HardDrive, Network, Database, Globe, ShieldAlert, ChevronLeft, ChevronRight } from 'lucide-react';
...
const AzureSecondarySidebar = () => {
  const [collapsed, setCollapsed] = useState(() => {
    try { return localStorage.getItem('sidebar-azure-collapsed') === 'true'; } catch { return false; }
  });

  const toggle = () => {
    const next = !collapsed;
    setCollapsed(next);
    try { localStorage.setItem('sidebar-azure-collapsed', String(next)); } catch {}
  };
  ...
  return (
    ...
    >
      ...
      <nav className="flex-1 px-1.5 py-2 space-y-0.5">
        {azureNavItems.map(({ to, label, icon: Icon, end }) => (
          <NavLink
            key={to}
            to={to}
            end={end}
...
export default AzureSecondarySidebar;
