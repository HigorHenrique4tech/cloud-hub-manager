...
import { Boxes, Layers, Share2, Network, Activity, ChevronLeft, ChevronRight } from 'lucide-react';
...
const K8sSecondarySidebar = () => {
  const [collapsed, setCollapsed] = useState(() => {
    try { return localStorage.getItem('sidebar-k8s-collapsed') === 'true'; } catch { return false; }
  });

  const toggle = () => {
    const next = !collapsed;
    setCollapsed(next);
    try { localStorage.setItem('sidebar-k8s-collapsed', String(next)); } catch {}
  };
  ...
  return (
    ...
    >
      ...
      <nav className="flex-1 px-1.5 py-2 space-y-0.5">
        {k8sNavItems.map(({ to, label, icon: Icon, end }) => (
          <NavLink
            key={to}
            to={to}
            end={end}
...
export default K8sSecondarySidebar;
