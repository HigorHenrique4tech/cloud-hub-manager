...
import { LayoutGrid, MonitorPlay, HardDrive, Database, Zap, Network, ChevronLeft, ChevronRight } from 'lucide-react';
...
const AwsSecondarySidebar = () => {
  const [collapsed, setCollapsed] = useState(() => {
    try { return localStorage.getItem('sidebar-aws-collapsed') === 'true'; } catch { return false; }
  });

  const toggle = () => {
    const next = !collapsed;
    setCollapsed(next);
    try { localStorage.setItem('sidebar-aws-collapsed', String(next)); } catch {}
  };
  ...
  return (
    ...
    >
      ...
      <nav className="flex-1 px-1.5 py-2 space-y-0.5">
        {awsNavItems.map(({ to, label, icon: Icon, end }) => (
          <NavLink
            key={to}
            to={to}
            end={end}
...
export default AwsSecondarySidebar;
