...
const PaymentMethodModal = ({ plan, onClose, onConfirm, loading }) => {
  ...
  const validateTaxId = () => {
    ...
    if (digits.length !== 11 && digits.length !== 14) {
      setTaxIdError('CPF deve ter 11 dígitos ou CNPJ 14 dígitos.');
      return false;
    }
    return true;
  };
        ...
        <div className="flex items-center justify-between border-b border-gray-700 px-5 py-4">
          <div>
            ...
            <p className="text-xs text-gray-400 mt-0.5">
              {plan.name} · <span className="text-white font-medium">R$ {(plan.amountCents / 100).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}/mês</span>
            </p>
          </div>
        ...
        <div className="px-5 py-4 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            >
              ...
              <div>
                ...
                <p className="text-[11px] text-gray-400 mt-0.5">Parcele em até 12x</p>
            ...
            <div>
              ...
              {installments > 1 && (
                <p className="text-xs text-gray-400 mt-1.5">
                  {installments}x de <span className="text-white font-medium">R$ {installmentValue.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</span>
                  {' '}sem juros
                </p>
              )}
            </div>
          ...
          <div>
            <label className="block text-xs font-medium text-gray-400 mb-1.5">
              ...
              <span className="ml-1 text-gray-500 font-normal">(obrigatório para emissão de nota)</span>
            </label>
            <input
              type="text"
              value={taxId}
              ...
              className={`w-full rounded-lg border px-3 py-2.5 text-sm text-white bg-gray-800 focus:outline-none transition-colors ${
                taxIdError ? 'border-red-500 focus:border-red-400' : 'border-gray-700 focus:border-primary'
              }`}
            />
            {taxIdError && <p className="text-xs text-red-400 mt-1">{taxIdError}</p>}
          </div>
