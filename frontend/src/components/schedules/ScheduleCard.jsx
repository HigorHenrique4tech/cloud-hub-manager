      ...
      <div className="flex items-start gap-3 p-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <span className={`rounded px-1.5 py-0.5 text-xs font-medium ${PROVIDER_BADGE[schedule.provider] || 'bg-slate-700 text-slate-300'}`}>
              {schedule.provider?.toUpperCase()}
            </span>
            <span className="text-sm font-semibold text-slate-100 truncate">{schedule.resource_name}</span>
            <span className="text-xs text-slate-400">({schedule.resource_type})</span>
          </div>
          ...
          <div className="mt-1.5 flex items-center gap-2 text-xs text-slate-400">
            ...
            <span>
              ...
              {' · '}
              <span className="font-mono text-slate-300">{schedule.schedule_time}</span>
              {' UTC'}
          ...
          <div className="mt-1 flex items-center gap-2 text-xs text-slate-500">
            ...
            {schedule.next_run_at && (
              <span className="text-slate-600">· Próximo: {fmtDateTime(schedule.next_run_at)}</span>
            )}
          </div>

          {schedule.last_run_error && (
            <p className="mt-1 text-xs text-red-400 truncate">{schedule.last_run_error}</p>
          )}
        </div>
        ...
        <div className="flex items-center gap-1.5 shrink-0">
          <PermissionGate permission="resources.start_stop">
            ...
            <button
              ...
              className="rounded p-1.5 text-slate-400 hover:bg-slate-700 hover:text-slate-200 transition-colors disabled:opacity-50"
            >
              {schedule.is_enabled
            ...
            <button
              ...
              title="Excluir"
              className="rounded p-1.5 text-slate-400 hover:bg-red-900/40 hover:text-red-300 transition-colors disabled:opacity-50"
            >
              <Trash2 size={14} />
            </button>
          </PermissionGate>
