...
const AWS = () => {
  ...
  const fetchInstances = async (isRefresh = false) => {
    try {
      ...
      if (err.response?.status === 400) {
        ...
        setError(err.response?.data?.detail || err.message || 'Erro ao carregar instâncias EC2');
      }
    } finally {
      setLoading(false);
      setRefreshing(false);
  ...
  const handleRefresh = () => fetchInstances(true);
  ...
  if (loading) {
    return <Layout><LoadingSpinner text="Carregando instâncias EC2..." /></Layout>;
  }
  ...
  if (error && instances.length === 0) {
    return <Layout><ErrorMessage message={error} onRetry={fetchInstances} /></Layout>;
  }
  ...
  return (
    <Layout onRefresh={handleRefresh} refreshing={refreshing}>
      <div className="mb-6">
        ...
        <p className="text-gray-600">Gerencie suas instâncias EC2 (Total: {instances.length})</p>
      </div>
      ...
      <div className="flex justify-between items-center mb-6">
        <div className="flex space-x-2">
          <button onClick={() => setViewType('table')}
            className={`px-4 py-2 rounded-lg font-medium transition-colors ${viewType === 'table' ? 'bg-primary text-white' : 'bg-white text-gray-700 hover:bg-gray-50 border border-gray-300'}`}>
            Tabela
          </button>
          <button onClick={() => setViewType('grid')}
            className={`px-4 py-2 rounded-lg font-medium transition-colors ${viewType === 'grid' ? 'bg-primary text-white' : 'bg-white text-gray-700 hover:bg-gray-50 border border-gray-300'}`}>
            Grade
          </button>
        </div>
      ...
      <div className="card">
        {instances.length === 0 ? (
          ...
          <EC2Table instances={instances} onStart={handleStart} onStop={handleStop} loading={refreshing} />
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {instances.map((instance) => (
              <ResourceCard key={instance.instance_id} resource={instance} type="ec2" />
            ))}
          </div>
        )}
      </div>
